...
mf.level_shift = 2.0
mf.max_cycle = 200
mf.irrep_nelec = { "A1": (5, 4), 'B1': (2, 2), 'B2': (2, 2), 'A2': (1, 0) }
#mf.chkfile = 'mno.chk'
...
mf.kernel()
#print(f'ene: {mf.e_tot}')
mf.analyze()
tools.molden.from_mo(mol, 'tio.molden', mf.mo_coeff)
print(f'mol.nao: {mol.nao}')
print(f'mol.nelec: {mol.nelec}')
#exit(0)
#mf.mo_coeff = symm.symmetrize_orb(mol, mf.mo_coeff)
...
mc.fcisolver.scratchDirectory = "/rc_scratch/anma2640/tio/"
mc.mc1step(mo)
tools.molden.from_mo(mol, 'tio_cas.molden', mc.mo_coeff)
#exit(0)
